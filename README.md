...
游戏《碧蓝档案》活动「夏莱的总结算与联邦学生会」中"在库管理"翻牌玩法的概率计算器：根据已翻开的格子推断每个未翻开格子里出现各物品的概率。
...
- **物品放置模式**：从右侧卡片点击放置按钮进入模式，点击棋盘任意合法位置即放置；矩形物品有「竖」「横」两个按钮（按放置后的实际形状区分，竖在前），正方形物品只有一个「放置」按钮。
